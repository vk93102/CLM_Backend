"""
Initialize anchor clauses for clause classification.

Anchor clauses are pre-defined legal clause types with:
- Label and description
- Category (Legal, Financial, Operational, etc.)
- Example text
- Pre-computed embeddings for semantic similarity

Run with: python manage.py shell < ai/initialize_anchor_clauses.py
"""
...
# Define anchor clauses for contract classification
ANCHOR_CLAUSES = [
    {
...
def initialize_anchor_clauses():
    """Create or update anchor clauses with embeddings"""
    ...
    for clause_data in ANCHOR_CLAUSES:
        ...
        if not example_embedding:
            logger.warning(f"Failed to generate embedding for {clause_data['label']}")
            continue
        ...
        logger.info(f"{action} anchor clause: {clause_data['label']} ({clause_data['category']})")
        print(f"{action} anchor clause: {clause_data['label']} ({clause_data['category']})")
    
    # Count active anchors
    active_count = ClauseAnchor.objects.filter(is_active=True).count()
    logger.info(f"Initialization complete. Total active anchor clauses: {active_count}")
    print(f"\nInitialization complete. Total active anchor clauses: {active_count}")
